...
export async function GET(
  ...
  try {
    ...
    // Query registries to build the staffing matrix for NON-OPERATIONAL shifts
    // This includes both regular non-operational shifts and special codes
    const query = `
      SELECT
        CASE 
          WHEN special_code IS NOT NULL THEN special_code
          ELSE CONCAT(
            shift_type_code,
      ...
      WHERE load_id = ?
      AND (
        registry_counts_as_operational = FALSE
        OR special_code IS NOT NULL
      )
    ...
    console.error("Non-operational staffing matrix error:", errorMessage);

    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch non-operational staffing matrix",
        error: errorMessage,
      },
      { status: 500 }
    );
